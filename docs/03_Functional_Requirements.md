...
| FR-AUD-002 | Audit records shall include actor, household, action, resource, timestamp, correlation ID, result, and safe metadata while excluding prohibited secrets and unnecessary raw values. | AUD-002, SEC-003 | Must | Phase 1 onward | Audit design | Logging tests prove required fields exist and prohibited fields are absent. |
