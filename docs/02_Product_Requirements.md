...
| PR-007 | Security, household isolation, auditability, and privacy must be designed from the start. | Must |

## 3. Identity, access, and household requirements

| ID | Requirement | Priority |
| --- | --- | --- |
| IAM-001 | Registration must be administrator-controlled or invitation-based. | Must |
| IAM-002 | The system must support login, logout, password change, account activation/deactivation, short-lived access tokens, and refresh-token rotation. | Must |
| IAM-003 | Passwords must be hashed with Argon2. | Must |
| IAM-004 | Owner, Administrator, Family Member, and Viewer roles must be supported with backend-enforced permissions. | Must |
| IAM-005 | Every protected record must be scoped to an authorised household where applicable. | Must |
| IAM-006 | Tests must prove that one household cannot access another household's records. | Must |
| HH-001 | A household must support name, profile, base currency, timezone, preferred language, members, roles, farming locations, financial-year preference, units, and notes. | Must |
| HH-002 | Base currency must be configurable; MMK may be the initial default. | Must |
...
| ID | Requirement | Priority |
| --- | --- | --- |
| DASH-001 | The dashboard must present understandable household, cash-flow, farm, crop, remittance, debt, receivable, and planning KPIs. | Must |
...
| RPT-006 | Exports must enforce household authorisation, safe temporary storage, filename sanitisation, path-traversal protection, logging, and deletion after use. | Must |
...
| I18N-001 | User-facing text must be externalised from components and support Shan initially, followed by Myanmar, English, and Japanese. | Must |
...
| AUD-002 | Audit records must contain actor, household, action, resource, timestamp, correlation ID, result, and safe metadata without unnecessary raw financial values. | Must |
...
This initial documentation task is complete when:
...
- no frontend or backend application code is added.
