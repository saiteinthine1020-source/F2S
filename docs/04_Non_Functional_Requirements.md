...
- **Provisional:** the target is reasonable for the initial household deployment but must be confirmed in the named validation issue or production-like environment.
...
| --- | --- | --- | --- | --- |
| Reference household | Up to 10 active members, 10 years of history, 100,000 finance events, 5,000 farming investments, and 50,000 related cost/harvest/sale/payment records | Provisional | Exceeds the expected first-family dataset while exposing inefficient list and aggregate designs. | Issue #8 shall validate entity volumes; performance fixtures shall generate the approved distribution. |
| Reference interactive load | 10 concurrent authenticated sessions for one or more isolated households | Provisional | Provides initial headroom above expected simultaneous family use without implying internet-scale capacity. | Issue #15 and the test strategy shall define a production-like load environment and workload mix. |
...
| --- | --- | --- | --- | --- |
| NFR-SEC-001 | 100 percent of protected endpoint, service, repository, aggregate, report, download, and AI-preparation test cases shall reject cross-household access. | Required | Household isolation is the primary confidentiality boundary. | Automated positive and negative isolation suites using at least two households and identifier substitution. |
...
| NFR-SEC-008 | 100 percent of supported file uploads and generated downloads shall validate authorisation, type/size policy, safe name/path handling, and expiry. | Required | Uploads and exports introduce traversal, malware, disclosure, and storage risks. | Malicious filename, traversal, content-type, oversize, expired-link, and cross-household test matrix. |
...
| NFR-PRIV-006 | Production, support, test, analytics, and documentation environments shall use zero real household financial data unless a separately approved protected-data procedure exists. | Required | Unnecessary copies increase disclosure risk. | Fixture review, environment audit, repository scan, and release checklist. |
...
| ID | Target | Status | Rationale | Verification method |
| --- | --- | --- | --- | --- |
| NFR-A11Y-001 | Supported critical flows shall have zero known WCAG 2.2 Level A or AA failures at release. | Required | Family members must be able to use essential finance and farming workflows. | Automated accessibility scans plus manual keyboard, screen-reader, zoom, contrast, focus, and error-state review. |
...
| NFR-OBS-003 | System clocks and persisted event timestamps shall be UTC and timezone-aware; user display shall use household timezone. | Required | Finance periods and audit order cannot depend on server locale. | Timestamp schema tests, timezone-boundary fixtures, and deployment configuration review. |
